...
class Generator(nn.Module):
    def __init__(
        ...
        size_ = 2 ** int(math.log(size, 2))
        if size != size_:
            self.last_up = nn.ConvTranspose2d(n_channels, n_channels, size - size_, 1)
        else:
            self.last_up = None
    ...
    def forward(self, z, return_latents = False, no_repeat = False, truncation = 1, truncation_latent = None, input_is_latent = False):
...
from networks_stylegan2 import MappingNetwork
    
class Discriminator(nn.Module):
    def __init__(self, size, c_dim = 0, channel_multiplier=2, out_shape = 1, blur_kernel=[1, 3, 3, 1], n_channels = 1):
        super().__init__()
        ...
        size_ = 2 ** int(math.log(size, 2))
        if size != size_:
            self.first_up = nn.Conv2d(n_channels, n_channels, size - size_, 1)
        else:
            self.first_up = None
    ...
    def forward(self, input, c = None):
        ...
        if c is not None:
            ...
            out = (out * cmap) * (1 / np.sqrt(out.shape[-1]))
        out = self.final(out)
