...
from viz import count_parameters
...
def main():
    ...
    model = NestedUNet(int(config.get('model_params', 'n_classes')), 2, filter_multiplier = float(args.filter_multiplier), small = args.small)
    ...
    criterion = torch.nn.BCEWithLogitsLoss(pos_weight = torch.FloatTensor([float(config.get('training_params', 'pos_bce_logits_weight'))]).to(device))
    optimizer = torch.optim.Adam(model.parameters(), lr = float(config.get('training_params', 'lr')))
